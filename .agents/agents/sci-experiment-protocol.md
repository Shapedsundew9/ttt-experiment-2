...
[Exact command: e.g., python -m python.experiments.exp_2025_014a_flux_conservation.run --config python/experiments/exp_2025_014a_flux_conservation/config.toml or cargo test -p exp_2025_014a_flux_conservation]
...
[Automated reduction target: The reduction script (e.g., python/scripts/reduce_telemetry.py) MUST parse raw telemetry from data/telemetry/ and format verified observables and falsification gates directly as a GitHub-Flavored Markdown table appended/injected into the Run Manifest (docs/research/runs/RUN-EXP-*.md). It MUST NOT emit loose JSON files as inter-agent evaluation handoffs.]
```
